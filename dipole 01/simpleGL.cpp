...
static FilterBox*	pFilterBox = NULL;
int					g_mousex = 0, g_mousey = 0;
int					g_width = 512, g_height = 512;
int					g_filterresolution = 256;
int					g_hfiltersize = 128;
float               g_bfactor = 0.6;
...
const char pixelShader[] =
{
	...
	"   \n"
	"    vec4 colorNorm = float4(color2.r/0.9, color2.g/0.6,color2.b/0.4,1.0f);	// 	should tend to make red particles and yellow at the brightest\n"
...
extern "C" 
void launch_kernel(float4* pos, float4* vel, unsigned int mesh_width, unsigned int mesh_height, float time, bool init);
...
bool initGL(int *argc, char **argv)
{
   ...
   m_vertexShader = glCreateShader(GL_VERTEX_SHADER);
	...
	if(!pFilterBox)
    {
	  ...
	  //
	  // here you specify the size of the fbo
	  // where the Gaussian filtering will happen
	  //
      pFilterBox->Initialize(g_filterresolution, g_filterresolution);
    }
...
void runCuda(struct cudaGraphicsResource **vbo_resource, struct cudaGraphicsResource **vbo_resource2)
{
    ...
    launch_kernel(dptr, dptr2,mesh_width, mesh_height, g_fAnim, first_time);
...
void display()
{
    ...
    runCuda(&cuda_vbo_resource, &cuda_vbo_resource2);
		pFilterBox->setWindowViewPort(0, 0, 512, 512);
	...
	//==============================================================================
	// Activate the filter box and render the glow objects again
	// note: this is not optimal, simply a quick way of implementing the effect

	pFilterBox->Activate(g_mousex - g_hfiltersize, g_mousey - g_hfiltersize, g_hfiltersize*2, g_hfiltersize*2); 
	...
	glUseProgram(m_program);      
	texLoc = glGetUniformLocation(m_program, "splatTexture");      
	glUniform1i(texLoc, 0);    
	glActiveTextureARB(GL_TEXTURE0_ARB);       
	glBindTexture(GL_TEXTURE_2D, m_texture);      
	glColor3f(1, 0, 0);
    ...
    // Draw the quad on top of the previous rendered scene.
    // using Blend ops
	pFilterBox->Draw(g_bfactor);
...
void cleanup()
{
	...
	if(pFilterBox) delete pFilterBox; // will call Destroy()
	pFilterBox = NULL;
}
